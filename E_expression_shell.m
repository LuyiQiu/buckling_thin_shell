function Energy = E_expression_shell (p,k,a,alpha) % Energy expression 
    ...
    e0=alpha*(p*D_area)/(2*pi) - k/(2*pi)*sum(y)*dt; %dimensionless parameter e0_bar

    Energy =0.5*integral (@(t) dphi(t,a).*dphi(t,a), 0, 2*pi) ...
    + 0.5*sum((e0+k.*y).^2)*dt - p * sum((1+alpha*e0+alpha*k*y).*x.*sin(theta+phi(theta,a)))*dt ...
    + p * pi * (1+alpha*e0+alpha*k*sum(y)*dt/(2*pi))...
    + a(5)*x(end);
end
